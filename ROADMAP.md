...
- **Project Initialization**: Setup Go 1.23 environment, `.gitignore`, and Linter configurations.
- **CI/CD Pipeline**: Implement GitHub Actions workflows for Go linting, unit testing (with race detector), and shell script validation.
- **TDD Infrastructure**: Establish 100% test coverage for configuration management (`pkg/config`).
- **Data Persistence**: Implement Pure-Go SQLite (`modernc.org/sqlite`) for zero-dependency local storage (`pkg/store`), and write CRUD unit tests.
- **Dependency Pinning**: Resolve Go Toolchain auto-upgrade issues and ensure stable CI builds.

## Phase 2: Minimum Viable Product (MVP)

- **API Integration**: Implement the `lumitree` API client (`pkg/client`) to fetch calendar meta-data and event lists.
- **Discord Bot Core**: Setup `bwmarrin/discordgo` session and graceful shutdown logic.
- **Slash Commands**:
